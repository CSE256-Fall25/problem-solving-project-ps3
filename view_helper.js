...
function define_new_regular_permissions(
  ...
  for (let g of which_groups) {
    let row = $(`
        <tr id="${id_prefix}_row_${g}" permission_group="${g}">
            <td id="${id_prefix}_checkcell_${g}" class="regularcheckcell" width="16px"></td>
            <td id="${id_prefix}_name_${g}" class="regular_perm_name">${g}</td>
        </tr>
        `);
  ...
  // call update_regular_contents when either username or filepath changes:
  define_attribute_observer(
    regular_container,
    "username",
    update_regular_contents
  );
  define_attribute_observer(
    regular_container,
    "filepath",
